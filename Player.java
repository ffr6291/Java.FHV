...
public class Player {
	...
	public void stats() {
		...
		_backpack.forEach(consumable -> {
			System.out.println("Backpack: " + consumable.getName()+"\n-----------------------------------------------\n");
		});
	...
	public void consume(Consumable consumable) {
		...
		if (this.isItemInBackpack(consumable)) {// only possible if item is in players backpack
			if (consumable.getHealingAmount() < 0) {
				...
				System.out.println("You drank an " + consumable.getName() + " and dealed yourselfe "
